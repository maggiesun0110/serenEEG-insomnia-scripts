...
output_path = "../results/features_with_ids.npz"
log_path = "../results/failures_with_ids.log"
...
features = []
labels = []
subject_ids = []  #NEW PART to keep track of subjevt ids
failed_subjects = []
...
for label, subdir in enumerate(["SC", "ST"]):
    ...
    for file in os.listdir(folder):
        if not file.endswith(".edf") or "Hypnogram" in file:
            continue
        try:
            subject_id = file.split("-")[0]  # SC4001E0
            prefix = subject_id[:-2]         # Remove suffix like E0, F0
            ...
            segment_len = segment_duration_sec * resample_rate  # 6000 samples per segment
            ...
            for i in range(n_segments):
                ...
                labels.append(label)  # 0 for SC, 1 for ST
                subject_ids.append(prefix)  # Save subject ID per segment
...
subject_ids = np.array(subject_ids)  # <<<convert list to np array
np.savez(output_path, X=features, y=labels, subject_ids=subject_ids)  # <<<IDs too
